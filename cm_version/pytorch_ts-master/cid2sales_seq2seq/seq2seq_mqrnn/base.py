...
class Feature_Flow_SKU:
    ...
    def create_train_test(self):
        ...
        if self.normalize:
            cols = ['instant_hour', 'redprice', 'instant_discount', 'nominal_discount', 'redprice_lag7', 'redprice_diff_percent'] + \
        ...
        return Xtrain.values, ytrain, Xtest.values, ytest, list(Xtrain.columns)
...
def main(data_dir, seed):
    ...
    def main_sub(features_df_sub, mode):
        ...
        Xtrain, ytrain, Xtest, ytest, feature_cols = feature_flow_sku.create_train_test()
    ...
    for mode in ['train', 'test']:
        encoder_arr, decoder_arr, targets_arr, start_points_arr, sku_brand_cid3_arr = [], [], [], [], []
        for i in trange(len(item_sku_id_list)):
            ...
            encoder_total, decoder_total, targets_total, start_points = main_sub(features_df_sub, mode)
