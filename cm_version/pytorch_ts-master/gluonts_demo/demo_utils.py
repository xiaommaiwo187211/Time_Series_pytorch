...
def plot_prediction(test_data_sample, test_data_pred_sample, sku, plot_days=120, plot=True):
    ...
    if plot:
        ...
        plt.axvline(x=fill_len, color='cornflowerblue', linestyle='--', alpha=0.5)
...
def generate_sku_listdataset(sku, sales_df, time_frame, cid3_dict, global_price_std):
    ...
    cols_to_normalize = ['instant_price', 'redprice', 'nominal_netprice']
    for col in cols_to_normalize:
        temp_mean, temp_std = Xtrain[col].mean(), Xtrain[col].std()
        temp_std = temp_std if temp_std > 0.01 else global_price_std
